...
If you happen to study physics at TUM and also need to learn the "Kurzfragen Katalog" for the Physik 3 exam, you can download the apkg file from the releases section.
...
The images then have to be placed in the assets/images folder. The images should be named in the following format: "0001.jpg", "0002.jpg", etc.
1. Then run ```python3 ./ocrImages.py```.
...
## How it works
The `ocrImages.py` script uses pytesseract to extract the text from the images. Then some filtering functions are applied to filter out only title and losung text. The coordinates of these text markers are used to crop the images. Then the cropped images are saved in the assets/output folder and the paths are saved in the index.json file. The index.json file is then used to generate the apkg files with the `generateAnki.py` script.
